...
func main() {
	...
	// 1. 创建一个文件服务器，指向网关本地素材缓存目录
	materialCacheDir := strings.TrimSpace(os.Getenv("GATEWAY_MATERIAL_CACHE_DIR"))
	if materialCacheDir == "" {
		materialCacheDir = "storage/materials"
	}
	fileServer := http.FileServer(http.Dir(materialCacheDir))
