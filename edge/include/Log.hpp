...
public:
    ...
    Log(std::string log_id, std::string device_id, std::string ad_id, std::string ad_file_name,
        ...
        std::string status_msg, long long created_at, std::string device_ip, std::string firmware_version, 
        std::string log_type = "PLAYBACK", int uploaded = 0)
        : log_id(std::move(log_id)), device_id(std::move(device_id)), ad_id(std::move(ad_id)),
          ...
          created_at(created_at), device_ip(std::move(device_ip)), firmware_version(std::move(firmware_version)), 
          log_type(std::move(log_type)), uploaded(uploaded) {}

    // JSON 序列化
    NLOHMANN_DEFINE_TYPE_INTRUSIVE(Log, log_id, device_id, ad_id, ad_file_name, start_time, end_time, 
                                   duration_ms, status_code, status_msg, created_at, device_ip, firmware_version, 
                                   log_type, uploaded)
...
private:
    ...
    std::string log_type; // PLAYBACK, CRASH, COMMAND, SYSTEM
